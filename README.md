...
- `src/TarjetaSube`: librería de clases con la implementación del dominio.
  - `Modelo/Tarjeta.cs`, `Modelo/Colectivo.cs`, `Modelo/Boleto.cs`
  - `Excepciones/`: excepciones de dominio (`CargaInvalidaException`, `SaldoInsuficienteException`)
  - `Datos/TarjetaSubeContext.cs`: `DbContext` con los `DbSet<Tarjeta>`, `DbSet<Colectivo>` y `DbSet<Boleto>`
- `tests/TarjetaSube.Tests`: proyecto de tests unitarios con NUnit (no dependen de una base de datos real).
